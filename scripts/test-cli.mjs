...
try {
  ...
  assert.match(baseRun.markdown, /## Category Scores/);
  assert.match(baseRun.markdown, /## What to fix first \/ 优先修复项/);
  ...
  assert.match(baseRun.markdown, /These files are generated locally and ignored by git by default/);
  ...
  assert.match(baseRun.html, /Runwise helps you check AI agent projects before they go live/);
  assert.match(baseRun.html, /Runwise 帮你在 AI Agent 项目上线前先做一次本地检查/);
  assert.match(baseRun.html, /Public preview · local-first/);
  assert.match(baseRun.html, /Overall score/);
  assert.match(baseRun.html, /Score overview/);
  assert.match(baseRun.html, /Rule Summary/);
  assert.match(baseRun.html, /Category Scores/);
  ...
  assert.match(baseRun.html, /Report Files \/ 报告文件/);
  assert.match(baseRun.html, /These files are generated locally and ignored by git by default/);
  ...
  assert.match(baseRun.html, /No project data is uploaded/);
...
async function assertViewerServer(projectRoot) {
  ...
  assert.match(dashboard.body, /Runwise helps you check AI agent projects before they go live/);
  assert.match(dashboard.body, /Runwise 帮你在 AI Agent 项目上线前先做一次本地检查/);
  assert.match(dashboard.body, /Generated by Runwise\. Local-first\. No project data is uploaded\./);
  assert.match(dashboard.body, /由 Runwise 生成。本地优先，不上传项目数据。/);
  assert.match(dashboard.body, /Overall score/);
  assert.match(dashboard.body, /Rule Summary/);
  assert.match(dashboard.body, /Detected Ecosystems \/ 检测到的生态/);
  assert.match(dashboard.body, /What to fix first/);
  assert.match(dashboard.body, /Findings Explorer/);
  assert.match(dashboard.body, /These files are generated locally and ignored by git by default/);
  assert.match(dashboard.body, /No eval coverage detected/);
  assert.doesNotMatch(dashboard.body, /\[object Object\]/);
