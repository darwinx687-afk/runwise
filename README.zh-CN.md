...
- 本地 Doctor 检查：工作区结构、包管理器状态、TypeScript 配置、治理文件、AI 迹象、MCP 迹象、评测覆盖和追踪覆盖。
- 生成 JSON 和 Markdown 报告，作为可审阅的就绪度证据。
- 为 Doctor finding 和 report 提供共享 TypeScript schema。
...
- Phase 2：追踪契约、回放 fixtures 和报告格式。
- Phase 3：集成 MCP Server、Agent 框架和 CI。
- Phase 4：完善 Dashboard 和文档。

## 贡献

Runwise 在 Phase 1 会刻意保持小范围。提交变更前，请阅读 [CONTRIBUTING.md](./CONTRIBUTING.md)、[PROJECT_CONSTITUTION.md](./PROJECT_CONSTITUTION.md) 和 [CODEX_LOOP_PROTOCOL.md](./CODEX_LOOP_PROTOCOL.md)。
